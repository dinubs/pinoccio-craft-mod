...
public class PinoccioAPI {
    public void turnLEDOn(int scoutID, int troopID, String token) throws Exception {
        requestAPI(String.format("https://api.pinocc.io/v1/%s/%s/command/led.on?token=%s", scoutID, troopID, token));
    }

    public void turnLEDOff(int scoutID, int troopID, String token) throws Exception {
        requestAPI(String.format("https://api.pinocc.io/v1/%s/%s/command/led.off?token=%s",scoutID,troopID,token));
    }
    public JsonArray troopsInAccount(String token) throws JsonSyntaxException, Exception {
    ...
    public JsonArray scoutsInTroop(int troopID, String token) throws JsonSyntaxException, Exception {
    ...
    public boolean runBitlashCommand(int troopID, int scoutID, String command, String token) throws Exception{
        String url = null;
        try {
//             url = URLEncoder.encode(command, "UTF-8");
        JsonElement jElement = new JsonParser().parse(requestAPI(String.format("https://api.pinocc.io/v1/%s/%s/command/%s?token=%s",troopID,scoutID,command,token)));
...
//        return jElement.getAsJsonObject().get("data");
    }
    public JsonElement runJsonBitlashCommand(int troopID, int scoutID, String command, String token) throws IOException{
        String url = null;
        try {
//             url = URLEncoder.encode(command, "UTF-8");
        	JsonElement jElement = new JsonParser().parse(requestAPI(String.format("https://api.pinocc.io/v1/%s/%s/command/%s?token=%s",troopID,scoutID,command,token)));
    ...
    private String requestAPI(String urlToRead) throws Exception {
        URL url;
